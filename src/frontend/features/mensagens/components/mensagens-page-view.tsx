...
export function MensagensPageView() {
  ...
  async function loadTemplates() {
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : "Erro ao carregar templates")
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleCreate(data: { title: string; body: string }) {
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : "Erro ao criar template")
    } finally {
      setCreating(false)
    }
  }

  async function handleUpdate(data: { title: string; body: string; order?: number }) {
    ...
    try {
      ...
      setTemplates((prev) => sortByOrder(prev.map((t) => (t.id === updated.id ? updated : t))))
      setEditTarget(null)
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : "Erro ao atualizar template")
    } finally {
      setUpdating(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      setDeleting(true)
      setErrorMessage("")
      const res = await fetch(`/api/mensagens/${deleteTarget.id}`, { method: "DELETE" })
      if (!res.ok) throw new Error("Não foi possível excluir template")
      setTemplates((prev) => sortByOrder(prev.filter((t) => t.id !== deleteTarget.id)))
      setDeleteTarget(null)
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : "Erro ao excluir template")
      setDeleteTarget(null)
    } finally {
      setDeleting(false)
    }
  ...
  return (
    <Stack spacing={3}>
      ...
      {loading ? (
        ...
        <Stack spacing={1.5}>
          {templates.length === 0 && (
            <Typography color="text.secondary">Nenhum template cadastrado ainda.</Typography>
          )}
              ...
              <CardContent>
                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 1 }}>
                  ...
                  <Box sx={{ display: "flex", flexShrink: 0 }}>
                    <IconButton size="small" onClick={() => setEditTarget(template)} aria-label="editar">
      ...
      <Dialog open={Boolean(editTarget)} onClose={() => setEditTarget(null)} fullWidth maxWidth="sm">
        ...
        <DialogContent sx={{ pt: 2 }}>
          {editTarget && (
            <MessageTemplateForm
              initial={{ title: editTarget.title, body: editTarget.body, order: editTarget.order }}
              showOrderField
              loading={updating}
