...
export function VisitantesList({ role, permissions }: VisitantesListProps) {
  ...
  const canVerMensagens =
    canEnviarMensagem || can(Permission.MENSAGENS_GERENCIAR);
    ...
    async function loadPage() {
      try {
        ...
        if (!response.ok || !result.items) {
          throw new Error(
            result.error?.message ?? "Nao foi possivel carregar visitantes",
          );
        }
  ...
  function handleApplyFilters() {
    if (
      ...
      setErrorMessage(
        "Periodo invalido: a data inicial deve ser menor ou igual a final",
      );
      return;
    }
  ...
  async function handleOpenDetail(id: string) {
    try {
      ...
      if (!response.ok || !result.visitante) {
        throw new Error(
          result.error?.message ?? "Nao foi possivel carregar detalhe",
        );
      }
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Erro ao abrir detalhe",
      );
      setSelectedId(null);
      setSelected(null);
    } finally {
  ...
  async function handleDelete() {
    ...
    try {
      setDeleting(true);
      const response = await fetch(`/api/visitantes/${selectedId}`, {
        method: "DELETE",
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(
          result.error?.message ?? "Nao foi possivel excluir visitante",
        );
      }
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Erro ao excluir visitante",
      );
      setDeleteConfirmOpen(false);
    } finally {
      setDeleting(false);
        ...
        >
          ...
          >
            <DatePicker
              ...
              onChange={(value) =>
                setDraftCreatedFrom(value ? value.format("YYYY-MM-DD") : "")
              }
              format="DD/MM/YYYY"
              slotProps={{
                textField: {
            ...
            <DatePicker
              ...
              onChange={(value) =>
                setDraftCreatedTo(value ? value.format("YYYY-MM-DD") : "")
              }
              format="DD/MM/YYYY"
              slotProps={{
                textField: {
            ...
            {canCreateVisitante && (
              <Button
                variant="contained"
                onClick={() => router.push("/visitantes/novo")}
              >
                Cadastrar visitante
              </Button>
            )}
                ...
                <CardActionArea onClick={() => void handleOpenDetail(item.id)}>
                  ...
                  >
                    <Box>
                      ...
                      <Typography color="text.secondary" variant="body2">
                        Telefone:{" "}
                        {item.phone ? formatPhone(item.phone) : "Nao informado"}
                      </Typography>
                    </Box>
            ...
            {modalLoading || !selected ? (
              ...
              <Stack spacing={1.5}>
                ...
                <Typography>
                  ...
                  {formatCivilDate(selected.member.birthDate)}
                </Typography>
                <Typography>
                ...
                <Typography>
                  ...
                  {translateActualChurch(selected.visitorProfile.actualChurch)}
                </Typography>
                <Typography>
                  <strong>Como conheceu:</strong>{" "}
                  {translateHowKnow(selected.visitorProfile.howKnow)}
                </Typography>
                  ...
                  <Stack spacing={0.8} sx={{ mt: 0.5 }}>
                    {selected.familyRelationships.map((item) => (
                      ...
                      >
                        ...
                        {translateRelationshipType(item.relationshipType)}
                      </Typography>
                    ))}
        ...
        >
          ...
          <DialogContent>
            <Typography>
              ...
              <strong>{selected?.member.name}</strong>? Esta acao nao pode ser
              desfeita.
            </Typography>
          </DialogContent>
          <DialogActions>
