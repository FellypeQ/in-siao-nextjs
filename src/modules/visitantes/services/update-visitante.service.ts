...
export async function updateVisitanteService(input: UpdateVisitanteInput) {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const existingMemberPrays = await findMemberPraysByMemberIdRepository(
        input.id,
        tx,
      );
      ...
      if (input.prayText) {
        ...
        await createMemberPrayRepository(
          { memberId: input.id, prayId: pray.id },
          tx,
        );
      }
      ...
      for (const operation of input.familyOperations) {
        ...
        const relationship = await findMemberRelationshipByIdRepository(
        ...
        if (!relationship || relationship.principalMemberId !== input.id) {
          throw new AppError(
            "Relacionamento familiar invalido",
            400,
        ...
        if (operation.action === "unlink") {
          await deleteMemberRelationshipRepository(
            operation.relationshipId,
            tx,
          );
          await ensureMemberVisitorProfileRepository(
        ...
        const relationshipCount =
          await countMemberRelationshipsByMemberIdRepository(
            operation.memberId,
            tx,
          );
