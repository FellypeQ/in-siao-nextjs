...
describe("/api/visitantes/[id] route", () => {
  ...
  it("GET retorna detalhe", async () => {
    getVisitanteDetailServiceMock.mockResolvedValueOnce({
      member: { id: "member-1" },
    });

    const response = await GET(
      new Request("http://localhost/api/visitantes/member-1"),
      {
  ...
  describe("DELETE /api/visitantes/[id]", () => {
    ...
    it("retorna 404 quando visitante nao encontrado", async () => {
      ...
      const response = await DELETE(
        new Request("http://localhost/api/visitantes/id-inexistente"),
        {
