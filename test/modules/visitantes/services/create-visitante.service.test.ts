...
vi.mock("@/modules/visitantes/repositories/create-member-visitor.repository", () => ({
  createMemberVisitorRepository: (...args: unknown[]) => createMemberVisitorRepositoryMock(...args)
}))

vi.mock("@/modules/visitantes/repositories/create-pray.repository", () => ({
  createPrayRepository: (...args: unknown[]) => createPrayRepositoryMock(...args)
}))

vi.mock("@/modules/visitantes/repositories/create-member-pray.repository", () => ({
  createMemberPrayRepository: (...args: unknown[]) => createMemberPrayRepositoryMock(...args)
}))
...
describe("createVisitanteService", () => {
  ...
  it("cria visitante principal e familiares", async () => {
    ...
    createMemberVisitorRepositoryMock.mockResolvedValueOnce({ id: "visitor-profile" })
    createPrayRepositoryMock.mockResolvedValueOnce({ id: "pray-1" })
    createMemberPrayRepositoryMock.mockResolvedValueOnce({ memberId: "member-principal", prayId: "pray-1" })
    createMemberRelationshipRepositoryMock.mockResolvedValueOnce({ id: "rel-1" })
