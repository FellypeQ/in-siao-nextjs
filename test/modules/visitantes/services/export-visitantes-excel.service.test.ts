...
vi.mock(
  "@/modules/visitantes/repositories/list-visitantes-for-export.repository",
  () => ({
    listVisitantesForExportRepository: (input: unknown) =>
      listVisitantesForExportRepositoryMock(input),
  }),
);
...
describe("exportVisitantesExcelService", () => {
  ...
  it("mantem abas existentes e adiciona Todos os visitantes sem IDs", async () => {
    ...
    expect(
      workbook.Sheets["Todos os visitantes"]?.["!autofilter"],
    ).toBeTruthy();

    const visitantesRows = XLSX.utils.sheet_to_json(
      workbook.Sheets.Visitantes,
      {
        header: 1,
      },
    ) as string[][];
    const familiaresRows = XLSX.utils.sheet_to_json(
      workbook.Sheets.Familiares,
      {
        header: 1,
      },
    ) as string[][];
    const allRows = XLSX.utils.sheet_to_json(
      workbook.Sheets["Todos os visitantes"],
      {
        header: 1,
      },
    ) as string[][];
    ...
    expect(visitantesRows[0]).toContain("Data de cadastro");
